...
#     do
#         LOGDIR="$BASE_DIR/logs/siren/$DSET/sidelen_$SIDELEN/scratch"
...
for PRETRAINEPOCH in 500
    do
        for SOURCEIMGID in `seq 1 40`
            do
                LOADPATH="$BASE_DIR/logs/siren/DIV2K/sidelen_$SIDELEN/scratch/imid[$SOURCEIMGID]_Siren_3x256_init[pytorch]_fbs[None]_lr[0.0001]_fw[30.0]_hw[30.0].pt"
                unset imgid_map
                declare -A imgid_map

                for TARGETIMGID in `seq 1 $MAXITER`
                    do
                        LOGDIR="$BASE_DIR/logs/siren/$DSET/sidelen_$SIDELEN/$SOURCEIMGID/pretrain_${PRETRAINEPOCH}step"
