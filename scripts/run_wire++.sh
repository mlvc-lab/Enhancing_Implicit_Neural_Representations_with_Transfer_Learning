...
for PRETRAINEPOCH in 500
    do
        for SOURCEIMGID in `seq 1 40`
            do
                LOADPATH="$BASE_DIR/logs/wire++/DIV2K/sidelen_$SIDELEN/scratch/imid[$SOURCEIMGID]_WF_3x256_init[sine]_fbs[1.0]_lr[0.001]_omega[5.0]_scale[2.0]_omegaw[4.0].pt"
                unset imgid_map
                declare -A imgid_map
