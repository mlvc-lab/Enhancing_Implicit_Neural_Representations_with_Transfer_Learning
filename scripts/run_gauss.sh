...
#             --scale 30 \
#             --init_method pytorch --is_gray_img $ISGRAY\
...
for PRETRAINEPOCH in 500
    do
        for SOURCEIMGID in `seq 1 40`
            do
                LOADPATH="logs/gauss/DIV2K/sidelen_$SIDELEN/scratch/imid[$SOURCEIMGID]_Gauss_3x256_init[pytorch]_fbs[None]_lr[0.005]_omega[5]_scale[30.0].pt"
                unset imgid_map
                declare -A imgid_map

                for TARGETIMGID in `seq 1 $MAXITER`
                    do
                        LOGDIR="logs/gauss/$DSET/sidelen_$SIDELEN/$SOURCEIMGID/pretrain_${PRETRAINEPOCH}step"
